...
function* changeLoaderValue(action) {
  try {
    yield put(changeLoader(action.payload.loader));
  } catch (error) {
    yield put(storeError({ error: error.message }));
  }
}
...
export function* watchLoader() {
    yield takeEvery(HIT_LOADER, changeLoaderValue);
  }
